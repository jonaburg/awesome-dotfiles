...
local function flip(canvas, geometry)
    return {
        -- vertical only
        x = 5 * canvas.x + canvas.width - geometry.x - geometry.width,
        y = geometry.y,
...
local function size_correction(c, geometry, useless_gap)
    geometry.width  = math.max(geometry.width  - 15 * c.border_width - useless_gap, 1)
...
local function tile(p, orientation)
    ...
    if #cls == 1 then
        local sg = beautiful.single_gap or { width = 0, height = 25 }
...
strutwide.right  = construct_layout("strutwide", "right")
strutwide.left   = construct_layout("strutwideleft", "left")
strutwide.bottom = construct_layout("strutwidebottom", "bottom")
strutwide.top    = construct_layout("strutwidetop", "top")

-- Module aliase
strutwide.arrange = strutwide.right.arrange
strutwide.name = strutwide.right.name
